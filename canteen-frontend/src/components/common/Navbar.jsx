...
const Navbar = () => {
  const { user, logout, isAdmin, isCashier, isCustomer } = useAuth(); // Make sure isCustomer is included here
  ...
  const navigation = [
    { name: 'Menu', href: '/menu', show: true },
    { name: 'POS', href: '/pos', show: isAdmin || isCashier },
    { name: 'Dashboard', href: '/dashboard', show: isAdmin },
    { name: 'Inventory', href: '/inventory', show: isAdmin },
            ...
            <div className="flex h-16 justify-between">
              ...
              <div className="hidden sm:ml-6 sm:flex sm:items-center">
                {isCustomer && (
                  <Link
                    to="/cart"
                    className="relative rounded-full bg-white p-1 text-gray-400 hover:text-gray-500 mr-3"
                  >
              ...
              <div className="-mr-2 flex items-center sm:hidden">
                {isCustomer && (
                  <Link
                    to="/cart"
                    className="relative rounded-full bg-white p-1 text-gray-400 hover:text-gray-500 mr-2"
                  >
                    <ShoppingCartIcon className="h-6 w-6" />
                    {getItemCount() > 0 && (
                      <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-primary-600 text-xs text-white flex items-center justify-center">
            ...
            <div className="border-t border-gray-200 pb-3 pt-4">
              <div className="flex items-center px-4">
                <div className="flex-shrink-0">
                  <div className="h-10 w-10 rounded-full bg-primary-600 text-white flex items-center justify-center text-lg">
                    {user?.name?.charAt(0).toUpperCase()}
                  </div>
                </div>
                <div className="ml-3">
